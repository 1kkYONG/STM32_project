...
* **硬體環境**：STM32F103，TIM2 掛載於 APB1（TimerClock ≈ 72 MHz）。
* **目標設定**：1 秒中斷一次（1 Hz）。
...
* **分頻後頻率**：$72\text{ MHz} \div 7200 = 10\text{ kHz}$
* **溢位時間**：$10\text{ kHz} \div 10000 = 1\text{ Hz}$ (每秒溢位一次)

### 為什麼設定值都要 -1？

硬體暫存器的運作邏輯如下：
1.  **PSC (Prescaler)**：實際除數為 `PSC + 1`。想除以 7200，PSC 需設 `7200 - 1`。
2.  **ARR (Auto-Reload Register)**：CNT 從 0 數到 ARR，共經歷 `ARR + 1` 個數。想數 10000 下，ARR 需設 `10000 - 1`。
...
* **優點**：
    1.  **硬體濾波 (ClockFilter)**：自動濾除毛刺與 EMI 雜訊。
    2.  **不佔用 CPU**：計數由硬體完成，溢位才通知 CPU。
    3.  **精確**：不會因為中斷頻繁而遺漏計數。
